#!/usr/bin/env python
"""Streaming + conteos en vivo con startingOffsets=earliest y outputMode=update, con mediciones por trigger."""
import json
import threading
import time
...
PORT = 8012  # puerto para esta variante
METRICS_PATH = ROOT / "metrics_update_earliest.csv"

spark = (
    SparkSession.builder.appName("KafkaFeedbackLiveCountsUpdateEarliest")
...
query = parsed.writeStream.outputMode("update").foreachBatch(save_and_log).start()
...
def serve_http():
    ...
    print(f"HTTP (update/earliest) en http://0.0.0.0:{PORT}")
    httpd.serve_forever()


if __name__ == "__main__":
    threading.Thread(target=serve_http, daemon=True).start()
    print(f"Streaming update/earliest, métricas -> {METRICS_PATH}")
