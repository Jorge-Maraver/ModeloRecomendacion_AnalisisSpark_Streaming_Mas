#!/usr/bin/env python
"""Bot 'revista': solo mujeres 10-16 años, ocupación estudiante (escuela), ~250 msg/s por defecto."""
import argparse
import json
import random
import time
...
def make_payload():
    gender = "Mujer"
    occupation = "Estudiante (Escuela/Instituto)"
...
def main():
    ...
    prod = Producer({"bootstrap.servers": args.bootstrap, "client.id": "bot-revista"})
    interval = 1.0 / max(args.rate, 1e-6)
    total = 0
    print(f"Bot revista -> {args.topic} @ ~{args.rate} msg/s (count={args.count or 'inf'})")
