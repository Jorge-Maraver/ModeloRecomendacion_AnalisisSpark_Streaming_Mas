#!/usr/bin/env python
"""Bot aleatorio (~50 msg/s por defecto)."""
import argparse
import json
import random
import time
...
def main():
    ...
    prod = Producer({"bootstrap.servers": args.bootstrap, "client.id": "bot-random50"})
    interval = 1.0 / max(args.rate, 1e-6)
    total = 0
    print(f"Bot random -> {args.topic} @ ~{args.rate} msg/s (count={args.count or 'inf'})")
